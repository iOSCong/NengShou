...
@interface FooterCollectionViewController () <RequestDataDelegate>
...
@implementation FooterCollectionViewController
...
//    self.collect.bounces = NO;
    ...
    [[RequestData alloc] initWithURLString:@"http://price.cartype.kakamobi.com/api/open/car-type-basic/get-compete-car-serial.htm" parameterDic:@{@"serialId":[NSString stringWithFormat:@"%@",self.hotCars.id]} delegate:self];
...
- (void)layoutCollectionView
{
    ...
    self.collect = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, self.view.bounds.size.width, 80) collectionViewLayout:layout];
//    [layout release];
    //配置属性
...
{
    FooterDetCollectionViewController *carsDetailVC = [[FooterDetCollectionViewController alloc] init];
    HotCars *hotCars = self.collectionArr[indexPath.row];
