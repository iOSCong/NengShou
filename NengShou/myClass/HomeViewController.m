...
- (void)addViewShow
{
    [self.navigationController pushViewController:[[NewsListViewController alloc] init] animated:YES];
}
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    if (_intType == 1) {
        ...
        if (indexPath.section == 0) {
            if ([self.rowArr0[indexPath.row][@"id"] intValue] == 4) {
                AllCarsTableViewController *vc = [[AllCarsTableViewController alloc] init];
                vc.title = mzstring(self.rowArr0[indexPath.row][@"title"]);
                [self.navigationController pushViewController:vc animated:YES];
            }else if ([self.rowArr0[indexPath.row][@"id"] intValue] == 6) {
...
//                    NSString *class = mzstring(self.rowArr0[indexPath.row][@"class"]);
//                    UIViewController* vc = [NSStrObject stringChangeToClass:class];
//                    if (vc) {
//                        [vc setValue:self.rowArr0[indexPath.row][@"id"] forKey:class];
//                        [self.navigationController pushViewController:vc animated:YES];
//                    }
